...
class DFHQLearner(FlYMazeAgent):
    def init_variables(self, alpha_r=0.1, kappa_r=0.1, alpha_h=0.1, weight_r=0.1, weight_h=0.1, weight_b=0.0):
        """
        Initialize variables for the Forgetting Habitual Q-Learning agent
        ======================================================
        ...
        Parameters:
        alpha_r: learning rate for reward value (float)
        kappa_r: forgetting rate for reward value (float)
        alpha_h: learning rate for habit value (float)
        weight_r: weight for reward value (float)
        weight_h: weight for habit value (float)
        weight_b: bias (float)
        """
        self.alpha_r = alpha_r  # Reward learning rate
        self.kappa_r = kappa_r  # Reward forgetting rate
        self.alpha_h = alpha_h  # Habit learning rate
        self.weight_r = weight_r  # Arbiter weight for reward
        self.weight_h = weight_h  # Arbiter weight for habits
        self.weight_b = weight_b  # Arbiter bias
    ...
    def update_knowledge(self, state, action, new_state, reward):
        ...
        reward: reward received (float)
        """
        self.r_table[state, :action] = (1 - self.kappa_r) * self.r_table[state, :action]  # update reward values
        self.r_table[state, action] = (1 - self.alpha_r) * self.r_table[state, action] + self.alpha_r * reward  # update reward values
        self.r_table[state, action + 1 :] = (1 - self.kappa_r) * self.r_table[state, action + 1 :]  # update reward values
...
class DFHQLearner_egreedy(DFHQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, epsilon=0.1):
        """
        Initialize variables for the epsilon-greedy policy
        ==================================================
...
class DFHQLearner_softmax(DFHQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, beta=0.2):
        """
        Initialize variables for the softmax policy
        ===========================================
    ...
    def vectorizedUpdate(
        self, action, reward, tables, alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta
    ):
        """
        Vectorized update of value-table using Forgetting Habitual Q-learning Algorithm
        ====================================================================
        ...
        tables: value tables (Theano tensor)
        alpha_r: learning rate for reward value (Theano tensor)
        kappa_r: forgetting rate for reward value (Theano tensor)
        alpha_h: learning rate for habit value (Theano tensor)
        weight_r: reward weight (Theano tensor)
        weight_h: habit weight (Theano tensor)
        weight_b: bias (Theano tensor)
        ...
        tables: updated value tables (Theano tensor)
        """
        tables = tt.set_subtensor(tables[: action], (1 - kappa_r) * tables[: action],)
        tables = tt.set_subtensor(tables[action + 1:self.action_space_size], (1 - kappa_r) * tables[action + 1 : self.action_space_size],)
        tables = tt.set_subtensor(tables[action], alpha_r * reward + (1 - alpha_r) * tables[action],)
        tables = tt.set_subtensor(
            tables[self.action_space_size : 2 * self.action_space_size],
            (1 - alpha_h) * tables[self.action_space_size : 2 * self.action_space_size],
        )
    ...
    def vectorizedActionProbabilities(
        self, alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, actions_set, rewards_set
    ):
        """
        Vectorized action probabilities for second alternative
        ======================================================
        ...
        Parameters:
        alpha_r: reward learning rate (Theano tensor)
        kappa_r: reward forgetting rate (Theano tensor)
        alpha_h: habit learning rate (Theano tensor)
        weight_r: reward weight (Theano tensor)
        weight_h: habit weight (Theano tensor)
        weight_b: bias (Theano tensor)
        ...
        for (actions_, rewards_) in zip(actions_set, rewards_set):  # for each session
            ...
            tables, _ = theano.scan(
                ...
                sequences=[actions, rewards],
                outputs_info=[tables],
                non_sequences=[alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta],
            )
    ...
    def fit(
        ...
        rewards_set: dataset of rewards (0 or 1) across multiple sessions (list of lists)
        alpha_r_params: Reward learning rate beta prior parameters (default: (1,1))
        kappa_r_params: Reward forgetting rate beta prior parameters (default: (1,1))
        alpha_h_params: Habit learning rate beta prior parameters (default: (1,1))
        weight_r_params: Reward weight normal prior parameters (default: (0,1))
        weight_h_params: Habit weight normal prior parameters (default: (0,1))
        weight_b_params: Bias normal prior parameters (default: (0,1))
        ...
        with pm.Model() as model:  # define model
            alpha_r = pm.Beta("alpha_r", alpha_r_params[0], alpha_r_params[1])  # reward learning rate
            kappa_r = pm.Beta("kappa_r", kappa_r_params[0], kappa_r_params[1])  # reward forgetting rate
            alpha_h = pm.Beta("alpha_h", alpha_h_params[0], alpha_h_params[1])  # habit learning rate
            weight_r = pm.Normal("weight_r", weight_r_params[0], weight_r_params[1])  # reward weight
            weight_h = pm.Normal("weight_h", weight_h_params[0], weight_h_params[1])  # habit weight
            weight_b = pm.Normal("weight_b", weight_b_params[0], weight_b_params[1])  # bias
            ...
            action_probabilities = self.vectorizedActionProbabilities(
                alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, actions_set, rewards_set
            )  # action probabilities
            like = pm.Bernoulli(
                "like", p=action_probabilities, observed=np.concatenate([a for a in actions_set])
            )  # Bernoulli likelihood
...
class DFHQLearner_esoftmax(DFHQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, epsilon=0.1, beta=0.2):
        """
        Initialize variables for the epsilon-softmax policy.
        ====================================================
    ...
    def vectorizedUpdate(
        self, action, reward, tables, alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, epsilon,
    ):
        """
        Vectorized update of Q-table using Forgetting Habitual Q-Learning model
        ============================================================
    ...
    def vectorizedActionProbabilities(
        self, alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, epsilon, actions_set, rewards_set,
    ):
        """
        Vectorized action probabilities for second alternative
        ======================================================
        ...
        Parameters:
        alpha_r: reward learning rate (float)
        kappa_r: reward forgetting rate (float)
        alpha_h: habit learning rate (float)
        weight_r: reward weight (float)
        weight_h: habit weight (float)
        weight_b: bias (float)
        ...
        for (actions_, rewards_) in zip(actions_set, rewards_set):  # for each session
            ...
            tables, _ = theano.scan(
                ...
                sequences=[actions, rewards],
                outputs_info=[tables],
                non_sequences=[alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, epsilon],
            )
...
class DFHQLearner_acceptreject(DFHQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy.
        ====================================================
    ...
    def vectorizedActionProbabilities(
        ...
        for (actions_, rewards_) in zip(actions_set, rewards_set):  # for each session
            ...
            tables, _ = theano.scan(
                ...
                sequences=[actions, rewards],
                outputs_info=[tables],
                non_sequences=[alpha_r, kappa_r, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, weight, intercept],
            )
...
class DFHCQLearner(FlYMazeAgent):
    def init_variables(self, alpha_r=0.1, kappa_r=0.1, gamma=0.9, alpha_h=0.1, weight_r=0.1, weight_h=0.1, weight_b=0.0):
        """
        Initialize variables for the Forgetting Habitual Q-Learning agent
        ======================================================
...
class DFHCQLearner_egreedy(DFHCQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, epsilon=0.1):
        """
        Initialize variables for the epsilon-greedy policy
        ==================================================
    ...
    def vectorizedUpdate(
        self, action, reward, tables, alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, epsilon
    ):
        """
        Vectorized update of value-table using Forgetting Habitual Q-learning Algorithm
        ====================================================================
        ...
        tables = tt.set_subtensor(tables[: action], (1 - kappa_r) * tables[: action],)
        tables = tt.set_subtensor(tables[action + 1:self.action_space_size], (1 - kappa_r) * tables[action + 1 : self.action_space_size],)
        tables = tt.set_subtensor(tables[action], alpha_r * (reward + gamma * next_best) + (1 - alpha_r) * tables[action],)
        tables = tt.set_subtensor(
            tables[self.action_space_size : 2 * self.action_space_size],
            (1 - alpha_h) * tables[self.action_space_size : 2 * self.action_space_size],
        )
    ...
    def vectorizedActionProbabilities(
        self, alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, epsilon, actions_set, rewards_set
    ):
        """
        Vectorized action probabilities for second alternative
        ======================================================
        ...
        for (actions_, rewards_) in zip(actions_set, rewards_set):  # for each session
            ...
            tables, _ = theano.scan(
                ...
                sequences=[actions, rewards],
                outputs_info=[tables],
                non_sequences=[alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, epsilon],
            )
...
class DFHCQLearner_softmax(DFHCQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, beta=0.2):
        """
        Initialize variables for the softmax policy
        ===========================================
    ...
    def vectorizedUpdate(
        self, action, reward, tables, alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta
    ):
        """
        Vectorized update of value-table using Forgetting Habitual Q-learning Algorithm
        ====================================================================
    ...
    def vectorizedActionProbabilities(
        self, alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, actions_set, rewards_set
    ):
        """
        Vectorized action probabilities for second alternative
        ======================================================
        ...
        for (actions_, rewards_) in zip(actions_set, rewards_set):  # for each session
            ...
            tables, _ = theano.scan(
                ...
                sequences=[actions, rewards],
                outputs_info=[tables],
                non_sequences=[alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta],
            )
    ...
    def fit(
        ...
        with pm.Model() as model:  # define model
            ...
            action_probabilities = self.vectorizedActionProbabilities(
                alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, actions_set, rewards_set
            )  # action probabilities
            like = pm.Bernoulli(
                "like", p=action_probabilities, observed=np.concatenate([a for a in actions_set])
            )  # Bernoulli likelihood
...
class DFHCQLearner_esoftmax(DFHCQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, epsilon=0.1, beta=0.2):
        """
        Initialize variables for the epsilon-softmax policy.
        ====================================================
    ...
    def vectorizedActionProbabilities(
        ...
        for (actions_, rewards_) in zip(actions_set, rewards_set):  # for each session
            ...
            tables, _ = theano.scan(
                ...
                sequences=[actions, rewards],
                outputs_info=[tables],
                non_sequences=[alpha_r, kappa_r, gamma, alpha_h, weight_r, weight_h, weight_b, theta_r, theta_h, beta, epsilon],
            )
    ...
    def fit(
        ...
        rewards_set: dataset of rewards (0 or 1) across multiple sessions (list of lists)
        alpha_r_params: Reward learning rate beta prior parameters (default: (1,1))
        kappa_r_params: Reward learning rate gamma prior parameters (default: (1,1))
        gamma_params: Discount factor beta prior parameters (default: (1,1))
        alpha_h_params: Habit learning rate beta prior parameters (default: (1,1))
        weight_r_params: Reward weight normal prior parameters (default: (0,1))
        weight_h_params: Habit weight normal prior parameters (default: (0,1))
        ...
        with pm.Model() as model:  # define model
            alpha_r = pm.Beta("alpha_r", alpha_r_params[0], alpha_r_params[1])  # reward learning rate
            kappa_r = pm.Beta("kappa_r", kappa_r_params[0], kappa_r_params[1])  # reward learning rate
            gamma = pm.Beta("gamma", gamma_params[0], gamma_params[1])  # discount factor
            alpha_h = pm.Beta("alpha_h", alpha_h_params[0], alpha_h_params[1])  # habit learning rate
            weight_r = pm.Normal("weight_r", weight_r_params[0], weight_r_params[1])  # reward weight
            weight_h = pm.Normal("weight_h", weight_h_params[0], weight_h_params[1])  # habit weight
...
class DFHCQLearner_acceptreject(DFHCQLearner):
    def init_policy_variables(self, theta_r=1, theta_h=1, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy.
        ====================================================
    ...
    def vectorizedUpdate(
        ...
        tables: value tables (Theano tensor)
        alpha_r: reward learning rate (Theano tensor)
        kappa_r: reward learning rate (Theano tensor)
        gamma: discount factor (Theano tensor)
        alpha_h: habit learning rate (Theano tensor)
        weight_r: reward weight (Theano tensor)
        weight_h: habit weight (Theano tensor)
    ...
    def vectorizedActionProbabilities(
        ...
        Parameters:
        alpha_r: reward learning rate (float)
        kappa_r: reward learning rate (float)
        gamma: discount factor (float)
        alpha_h: habit learning rate (float)
        weight_r: reward weight (float)
        weight_h: habit weight (float)
    ...
    def fit(
        ...
        rewards_set: dataset of rewards (0 or 1) across multiple sessions (list of lists)
        alpha_r_params: Reward learning rate beta prior parameters (default: (1,1))
        kappa_r_params: Reward learning rate beta prior parameters (default: (1,1))
        gamma_params: Discount factor beta prior parameters (default: (1,1))
        alpha_h_params: Habit learning rate beta prior parameters (default: (1,1))
        weight_r_params: Reward weight normal prior parameters (default: (0,1))
        weight_h_params: Habit weight normal prior parameters (default: (0,1))
