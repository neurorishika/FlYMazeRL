...
"""
+++++++++++++++++++++++++++++++
Variational Recurrent Neural Network Agents
+++++++++++++++++++++++++++++++
"""
...
class VariationalRNN(nn.Module):
    """
    A class to create a Variational Recurrent Neural Network agent with the option to use an intermediate Linear encoder.
    """
    ...
    def __init__(
        ...
        self.prior_mean = 0.0
        self.prior_std = 1.0
        # define input output and hidden state sizes
        ...
        if use_intermediate_encoder:
            ...
            self.feature_encoder = nn.Linear(input_size, encoder_size)
            self.feature_decoder = nn.Linear(encoder_size, input_size)
            self.encoder_size = encoder_size

            # define variational encoder
            self.variational_encoder = nn.Sequential(nn.Linear(2 * encoder_size, encoder_size), nn.ReLU(),)
            self.variational_encoder_mean = nn.Linear(encoder_size, state_size)
            self.variational_encoder_std = nn.Sequential(nn.Linear(encoder_size, state_size), nn.Softplus())

            # define variational decoder
            self.variational_decoder = nn.Sequential(nn.Linear(2 * encoder_size, encoder_size), nn.ReLU(),)
            self.variational_decoder_mean = nn.Sequential(nn.Linear(encoder_size, output_size), nn.Sigmoid())
            self.variational_decoder_std = nn.Sequential(nn.Linear(encoder_size, output_size), nn.Softplus())

            # recurrent layer
            self.rnn = nn.RNN(2 * encoder_size, encoder_size, num_layers, batch_first=True)
        else:
            ...
            # define variational encoder
            self.variational_encoder = nn.Sequential(nn.Linear(2 * state_size, state_size), nn.ReLU(),)
            self.variational_encoder_mean = nn.Linear(state_size, state_size)
            self.variational_encoder_std = nn.Sequential(nn.Linear(state_size, state_size), nn.Softplus())

            # define variational decoder
            self.variational_decoder = nn.Sequential(nn.Linear(2 * state_size, state_size), nn.ReLU(),)
            self.variational_decoder_mean = nn.Sequential(nn.Linear(state_size, output_size), nn.Sigmoid())
            self.variational_decoder_std = nn.Sequential(nn.Linear(state_size, output_size), nn.Softplus())

            # recurrent layer
            self.rnn = nn.RNN(2 * state_size, state_size, num_layers, batch_first=True)
    ...
    def _reparameterized_sample(self, mean, std):
        ...
        Parameters:
        mean: The mean of the normal distribution (torch.Tensor)
        std: The standard deviation of the normal distribution (torch.Tensor)

        Returns:
        sample: A sample from the normal distribution (torch.Tensor)
        """
    ...
    def forward(self, inputs, hidden_state, get_hidden_dynamics=False):
        ...
        if self.feature_encoder is not None:
            ...
            x_ = self.feature_encoder(x_)

            # loop through time steps
            for t in range(x.shape[0]):

                l_t = self.feature_encoder(x[t])
                l_t_ = self.feature_encoder(x_[t])

                # get variational parameters
                enc_x_t = torch.cat([l_t, h], dim=1)
                enc_x_t_ = torch.cat([l_t_, h], dim=1)

                enc_mean = self.variational_encoder_mean(self.variational_encoder(enc_x_t))
                enc_std = self.variational_encoder_std(self.variational_encoder(enc_x_t))

                dec_mean = self.variational_decoder_mean(self.variational_decoder(enc_x_t_))
                dec_std = self.variational_decoder_std(self.variational_decoder(enc_x_t_))

                # sample from variational distribution
                z_t = self._reparameterized_sample(enc_mean, enc_std)
                z_t_ = self._reparameterized_sample(dec_mean, dec_std)

                # update hidden state
                h = self.rnn(torch.cat([z_t, z_t_], dim=1).unsqueeze(0), h.unsqueeze(0))[1].squeeze(0)

            # get output
            output = self.variational_decoder_mean(self.variational_decoder(torch.cat([z_t, z_t_], dim=1)))
        else:
