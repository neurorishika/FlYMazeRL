...
class DEIQLearner(FlYMazeAgent):
...
class DEIQLearner_egreedy(DEIQLearner):
...
class DEIQLearner_softmax(DEIQLearner):
...
class DEIQLearner_esoftmax(DEIQLearner):
...
class DEIQLearner_acceptreject(DEIQLearner):
...
class DEIQLearnerHM(FlYMazeAgent):
    def init_variables(self, alpha=[0.1, 0.1], tau=[0.1, 0.1], init_q=[0.5, 0.5]):
        """
        Initialize variables for the Differential Extinction Q-Learning agent
        =====================================================================
    ...
    def update_knowledge(self, state, action, new_state, reward):
        ...
        """
        if reward != 0:
            self.q_table[state, action] += self.alpha[action] * (reward - self.q_table[state, action])
        else:
...
class DEIQLearnerHM_acceptreject(DEIQLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas, taus):
        """
        Vectorized update of Q-table using the Differential Extinction Q-learning algorithm
        ===================================================================================
    ...
    def vectorizedActionProbabilities(self, alphas, taus, init_q, weight, intercept, actions_set, rewards_set):
        """
        Vectorized action probabilities for second alternative
        ======================================================
    ...
    def fit(
        ...
        with pm.Model() as model:  # define model
            ...
            action_probabilities = self.vectorizedActionProbabilities(
                alphas, taus, init_q, weight, intercept, actions_set, rewards_set
            )  # action probabilities
