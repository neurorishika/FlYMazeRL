...
class IQLearnerHM(FlYMazeAgent):
    ...
    def trial_step(self, state):
        ...
        - Get reward and new state
        - Update according to the equation:
            Q[state, action] = (1 - alpha) * Q[state, action] + alpha[action] * reward
        ======================================================================================
...
class IQLearnerHM_acceptreject(IQLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas):
        ...
        Qs: updated Q-value (Theano tensor)
        """
        Qs = tt.set_subtensor(Qs[action], Qs[action] + alphas[action] * (reward - Qs[action]))
        return Qs
    ...
    def vectorizedActionProbabilities(self, alphas, init_q, weight, intercept, actions_set, rewards_set):
        ...
        Parameters:
        alphas: learning rates (Theano tensor)
        init_q: initial Q-value (Theano tensor)
