...
class RPEFreeLearner(FlYMazeAgent):
...
class RPEFreeLearner_egreedy(RPEFreeLearner):
...
class RPEFreeLearner_softmax(RPEFreeLearner):
...
class RPEFreeLearner_esoftmax(RPEFreeLearner):
...
class RPEFreeLearner_acceptreject(RPEFreeLearner):
...
class RPEFreeLearnerHM(FlYMazeAgent):
...
class RPEFreeLearnerHM_acceptreject(RPEFreeLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas):
        ...
        reward: reward received (0 or 1)
        Qs: current Q-value (Theano tensor)
        alphas: learning rates (Theano tensor)
        gamma: discount rate (float)
