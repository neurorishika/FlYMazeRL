...
class FIQLearner_egreedy(FIQLearner):
...
class FIQLearner_softmax(FIQLearner):
...
class FIQLearner_esoftmax(FIQLearner):
...
class FIQLearner_acceptreject(FIQLearner):
...
class FIQLearnerHM(FlYMazeAgent):
    ...
    def update_knowledge(self, state, action, new_state, reward):
        ...
        reward: reward received (float)
        """
        self.q_table[state, :] = (1 - self.alpha[action]) * self.q_table[state, :]
        self.q_table[state, action] += self.alpha[action] * reward  # update Q-table
...
class FIQLearnerHM_acceptreject(FIQLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
