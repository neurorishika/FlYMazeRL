...
class SARSALearnerHM(FlYMazeAgent):
...
class SARSALearnerHM_acceptreject(SARSALearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy.
        ====================================================
    ...
    def vectorizedUpdate(self, action, next_action, reward, Qs, alphas, gamma):
        """
        Vectorized update of Q-table using SARSA Algorithm.
        ===================================================
        ...
        Qs: updated Q-value (Theano tensor)
        """
        Qs = tt.set_subtensor(Qs[action], Qs[action] + alphas[action] * (reward + gamma * Qs[next_action] - Qs[action]))
        return Qs
    ...
    def vectorizedActionProbabilities(self, alphas, init_q, gamma, weight, intercept, actions_set, rewards_set):
        ...
        """
        probabilities = []
        for n, (actions_, rewards_) in enumerate(zip(actions_set, rewards_set)):
            rewards = theano.shared(np.asarray(rewards_[:-1], dtype="int16"))
            actions = theano.shared(np.asarray(actions_[:-1], dtype="int16"))
            next_actions = theano.shared(np.asarray(actions_[1:], dtype="int16"))
            ...
            # Apply the softmax function
            Qs_ = tt.concatenate([[init_q[n]], [init_q[n]], Qs], axis=0)
            Qs_ = 1 / (1 + tt.exp(-(weight * Qs_[:-1] + intercept)))
            log_prob_action_1 = (
                tt.log(Qs_[:, 1])
                + tt.log(3 - Qs_[:, 0])
