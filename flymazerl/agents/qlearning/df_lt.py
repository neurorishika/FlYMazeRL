...
class DFLTQLearner(FlYMazeAgent):
...
class DFLTQLearner_egreedy(DFLTQLearner):
...
class DFLTQLearner_softmax(DFLTQLearner):
...
class DFLTQLearner_esoftmax(DFLTQLearner):
...
class DFLTQLearner_acceptreject(DFLTQLearner):
...
class DFLTQLearnerHM(FlYMazeAgent):
    def init_variables(self, alpha=[0.1, 0.1], kappa=[0.1, 0.1], init_q=[0.5, 0.5], gamma=0.5):
        """
        Initialize variables for the Differential Forgetting Long-Term Q-Learning agent
        ===============================================================================
...
class DFLTQLearnerHM_acceptreject(DFLTQLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas, kappas, gamma):
        """
        Vectorized update of Q-table using Differential Forgetting Long-Term Q-learning algorithm
        =========================================================================================
        ...
        """
        next_best = tt.max(Qs)
        Qs = tt.set_subtensor(Qs[:action], (1 - kappas[:action]) * Qs[:action])  # update Q-table
        Qs = tt.set_subtensor(Qs[action], (1 - alphas[action]) * Qs[action] + alphas[action] * (reward + gamma * next_best))
        Qs = tt.set_subtensor(Qs[action + 1 :], (1 - kappas[action + 1 :]) * Qs[action + 1 :])
        return Qs

    def vectorizedActionProbabilities(self, alphas, kappas, init_q, gamma, weight, intercept, actions_set, rewards_set):
        """
        Vectorized action probabilities for second alternative
        ======================================================
        ...
        for n,(actions_, rewards_) in enumerate(zip(actions_set, rewards_set)):  # for each session
            ...
            Qs, _ = theano.scan(
                ...
                sequences=[actions, rewards],
                outputs_info=[Qs],
                non_sequences=[alphas[n], kappas[n], gamma[n]],
            )
    ...
    def fit(
        ...
        with pm.Model() as model:  # define model
            ...
            action_probabilities = self.vectorizedActionProbabilities(
                alphas, kappas, init_q, gamma, weight, intercept, actions_set, rewards_set
            )  # action probabilities
