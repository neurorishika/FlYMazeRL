...
class FRPEFreeLearner(FlYMazeAgent):
...
class FRPEFreeLearner_egreedy(FRPEFreeLearner):
...
class FRPEFreeLearner_softmax(FRPEFreeLearner):
...
class FRPEFreeLearner_esoftmax(FRPEFreeLearner):
...
class FRPEFreeLearner_acceptreject(FRPEFreeLearner):
...
class FRPEFreeLearnerHM(FlYMazeAgent):
    def init_variables(self, alpha=[0.1, 0.1], init_q=[0.5, 0.5]):
        """
        Initialize variables for the Forgetting Reward Learning agent
        =============================================
    ...
    def update_knowledge(self, state, action, new_state, reward):
        ...
        reward: reward received (float)
        """
        self.q_table[state, :action] = (1 - self.alpha[:action]) * self.q_table[state, :action]
        self.q_table[state, action] = self.q_table[state, action] + self.alpha[action] * reward  # update Q-table
        self.q_table[new_state, action + 1 :] = (1 - self.alpha[action + 1 :]) * self.q_table[new_state, action + 1 :]
...
class FRPEFreeLearnerHM_acceptreject(FRPEFreeLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas):
        """
        Vectorized update of Q-table using Reward learning algorithm
        =====================================================================
        ...
        Qs: updated Q-value (Theano tensor)
        """
        Qs = tt.set_subtensor(Qs[:action], (1 - alphas[:action]) * Qs[:action])
        Qs = tt.set_subtensor(Qs[action], Qs[action] + alphas[action] * reward)
        Qs = tt.set_subtensor(Qs[action + 1 :], (1 - alphas[action + 1 :]) * Qs[action + 1 :])
        return Qs

    def vectorizedActionProbabilities(self, alphas, init_q, weight, intercept, actions_set, rewards_set):
        """
        Vectorized action probabilities for second alternative
        ======================================================
        ...
        for n, (actions_, rewards_) in enumerate(zip(actions_set, rewards_set)):  # for each session
            ...
            Qs = init_q[n]
            Qs, _ = theano.scan(
                fn=self.vectorizedUpdate, sequences=[actions, rewards], outputs_info=[Qs], non_sequences=[alphas[n]]
            )
    ...
    def fit(
        ...
        with pm.Model() as model:  # define model
            ...
            action_probabilities = self.vectorizedActionProbabilities(
                alphas, init_q, weight, intercept, actions_set, rewards_set
            )  # action probabilities
