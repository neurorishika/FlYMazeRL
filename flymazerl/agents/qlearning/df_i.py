...
class DFIQLearner(FlYMazeAgent):
...
class DFIQLearner_egreedy(DFIQLearner):
...
class DFIQLearner_softmax(DFIQLearner):
...
class DFIQLearner_esoftmax(DFIQLearner):
...
class DFIQLearner_acceptreject(DFIQLearner):
...
class DFIQLearnerHM(FlYMazeAgent):
    def init_variables(self, alpha=[0.1, 0.1], kappa=[0.1, 0.1], init_q=[0.5, 0.5]):
        """
        Initialize variables for the Differential Forgetting Q-Learning agent
        =====================================================================
    ...
    def update_knowledge(self, state, action, new_state, reward):
        ...
        reward: reward received (float)
        """
        self.q_table[state, :action] = (1 - self.kappa[:action]) * self.q_table[state, :action]
        self.q_table[state, action] = (1 - self.alpha[action]) * self.q_table[state, action] + self.alpha[
            action
        ] * reward
        self.q_table[state, action + 1 :] = (1 - self.kappa[action + 1 :]) * self.q_table[state, action + 1 :]
...
class DFIQLearnerHM_acceptreject(DFIQLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas, kappas):
        """
        Vectorized update of Q-table using the Differential Forgetting Q-learning algorithm
        ===================================================================================
        ...
        Qs: updated Q-value (Theano tensor)
        """
        Qs = tt.set_subtensor(Qs[:action], (1 - kappas[:action]) * Qs[:action])
        Qs = tt.set_subtensor(Qs[action], (1 - alphas[action]) * Qs[action] + alphas[action] * reward)
        Qs = tt.set_subtensor(Qs[action + 1 :], (1 - kappas[action + 1 :]) * Qs[action + 1 :])
        return Qs

    def vectorizedActionProbabilities(self, alphas, kappas, init_q, weight, intercept, actions_set, rewards_set):
        """
        Vectorized action probabilities for second alternative
        ======================================================
        ...
        for n, (actions_, rewards_) in enumerate(zip(actions_set, rewards_set)):  # for each session
            ...
            Qs, _ = theano.scan(
                ...
                outputs_info=[Qs],
                non_sequences=[alphas[n], kappas[n]],
            )
    ...
    def fit(
        ...
        with pm.Model() as model:  # define model
            ...
            # forgetting rate hyperpriors
            kappa_a_mean = pm.Gamma("kappa_beta_a", kappa_params[0], kappa_params[1], shape=n_actions)
            kappa_b_mean = pm.Gamma("kappa_beta_b", kappa_params[0], kappa_params[1], shape=n_actions)
            # forgetting rate priors samples (for each individual flies and action)
            kappas = []
            for n in range(n_actions):
                kappas.append(pm.Beta("kappa_{}".format(n), kappa_a_mean[n], kappa_b_mean[n], shape=n_flies))
            # combine forgetting rates
            kappas = pm.Deterministic("kappa", tt.stack(kappas, axis=1))
            ...
            action_probabilities = self.vectorizedActionProbabilities(
                alphas, kappas, init_q, weight, intercept, actions_set, rewards_set
            )  # action probabilities
