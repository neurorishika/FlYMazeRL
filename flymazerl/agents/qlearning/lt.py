...
class LTQLearner_egreedy(LTQLearner):
...
class LTQLearner_softmax(LTQLearner):
...
class LTQLearner_esoftmax(LTQLearner):
...
class LTQLearner_acceptreject(LTQLearner):
...
class LTQLearnerHM(FlYMazeAgent):
...
class LTQLearnerHM_acceptreject(LTQLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy.
        ====================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas, gamma):
        ...
        Qs: updated Q-value (Theano tensor)
        """
        Qs = tt.set_subtensor(Qs[action], Qs[action] + alphas[action] * (reward + gamma * tt.max(Qs) - Qs[action]))
        return Qs
    ...
    def vectorizedActionProbabilities(self, alphas, init_q, gamma, weight, intercept, actions_set, rewards_set):
        ...
        for n, (actions_, rewards_) in enumerate(zip(actions_set, rewards_set)):  # for each session
            ...
            Qs, _ = theano.scan(
                ...
                outputs_info=[Qs],
                non_sequences=[alphas[n], gamma[n]],
            )
