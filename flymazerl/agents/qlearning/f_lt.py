...
class FLTQLearner(FlYMazeAgent):
...
class FLTQLearner_egreedy(FLTQLearner):
...
class FLTQLearner_softmax(FLTQLearner):
...
class FLTQLearner_esoftmax(FLTQLearner):
...
class FLTQLearner_acceptreject(FLTQLearner):
...
class FLTQLearnerHM(FlYMazeAgent):
    ...
    def update_knowledge(self, state, action, new_state, reward):
        ...
        self.q_table[state, action] += self.alpha[action] * (reward + self.gamma * next_best)
...
class FLTQLearnerHM_acceptreject(FLTQLearnerHM):
    def init_policy_variables(self, weight=1.0, intercept=0.0):
        """
        Initialize variables for the accept-reject policy
        ===================================================
    ...
    def vectorizedUpdate(self, action, reward, Qs, alphas, gamma):
        """
        Vectorized update of Q-table using Forgetting Long-Term Q-learning algorithm
        ============================================================================
        ...
        Qs = tt.set_subtensor(Qs[action], Qs[action] + alphas[action] * (reward + gamma * next_best))
        return Qs
    ...
    def vectorizedActionProbabilities(self, alphas, init_q, gamma, weight, intercept, actions_set, rewards_set):
        ...
        for n,(actions_, rewards_) in enumerate(zip(actions_set, rewards_set)):  # for each session
            ...
            Qs = init_q[n]
            Qs, _ = theano.scan(
                fn=self.vectorizedUpdate, sequences=[actions, rewards], outputs_info=[Qs], non_sequences=[alphas[n], gamma[n]]
            )
