...
argument_parser = argparse.ArgumentParser(
    description=start_text + "Script to fit the parameters of the a variety of NN-based RL agents to the a 2AFC maze task using Backpropagation (through time)."
)
...
argument_parser.add_argument(
    ...
    type=str,
    default="GQNN",
    help="Agent to use. Options: GQNN, GRNN",
)
argument_parser.add_argument(
    "--action-set-data",
    type=str,
...
argument_parser.add_argument("--num_reservoirs", type=int, default=1, help="Number of Reservoirs to use. (Only for GRNN)")
argument_parser.add_argument("--encoder_size", type=int, default=None, help="Encoder size to use. (Only for GRNN)")
argument_parser.add_argument("--kind", type=str, default="RNN", help="Kind of network to use. (Only for GRNN; options: RNN, LSTM, GRU)")

# Model parameters : GQNN
argument_parser.add_argument("--hidden_state_sizes", nargs="+", type=int, default=[10], help="Hidden state sizes to use. (Only for GQNN)")
argument_parser.add_argument("--activation", type=str, default="relu", help="Activation function to use. (Only for GQNN; options: relu, tanh, sigmoid)")

# Common parameters
argument_parser.add_argument("--policy_type", type=str, default="acceptreject", help="Policy type to use. (options: softmax, egreedy, acceptreject)")
argument_parser.add_argument("--device", type=str, default="cpu", help="Device to use. (Only for GRNN; options: cpu, cuda)")
argument_parser.add_argument("--symmetric", type=bool, default=True, help="Whether to use symmetric or asymmetric networks.")
argument_parser.add_argument("--allow_negative", type=bool, default=True, help="Whether to allow negative Q values.")
argument_parser.add_argument("--omission_is_punishment", type=bool, default=False, help="Whether to use omission as a punishment.")
...
results_df = pd.DataFrame(
    columns=[
        'ModelID','EnsembleID','FoldID','FlyID','MSE(train)','NMSE(train)','LogLikelihood(train)','AIC(train)','BIC(train)','MSE(test)','NMSE(test)','LogLikelihood(test)','AIC(test)','BIC(test)','P(action)','Model Path'
    ]
)
...
for ensemble_id in range(args.n_ensemble):
    ...
    for fold_id,(train_set,val_set) in enumerate(zip(training_sets,validation_sets)):
        ...
        agent.fit(train_set['actions'], train_set['rewards'], **fit_params)
        ...
        for i,fly_id in enumerate(train_set['indices']):
            ...
            obs_action_smooth = np.convolve(obs_actions,np.ones((args.history_size,))/args.history_size,mode='full')[args.history_size:]
            pred_action_prob = p_action_train[i,:]
            pred_action_smooth = np.convolve(pred_action_prob,np.ones((args.history_size,))/args.history_size,mode='full')[args.history_size:]
            ...
            # append to dataframe
            results_df.loc[len(results_df)] = [parameter_string ,ensemble_id,fold_id,fly_id,mse_train,nmse_train,ll_train,aic_train,bic_train,np.NaN,np.NaN,np.NaN,np.NaN,np.NaN,pred_action_prob.tolist(),"ensemble_{}_fold_{}".format(ensemble_id,fold_id)]
        ...
        for i,fly_id in enumerate(val_set['indices']):
            ...
            # calculate bernoulli log likelihood of the actions given the predicted action probabilities
            ll_val = np.sum(np.log(pred_action_prob[obs_actions==1])) + np.sum(np.log(1-pred_action_prob[obs_actions==0]))
            ...
            # append to dataframe
            results_df.loc[len(results_df)] = [parameter_string ,ensemble_id,fold_id,fly_id,np.NaN,np.NaN,np.NaN,np.NaN,np.NaN,mse_val,nmse_val,ll_val,aic_val,bic_val,pred_action_prob.tolist(),"ensemble_{}_fold_{}".format(ensemble_id,fold_id)]
